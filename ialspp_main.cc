...
/**
 * @brief Projects a block 
 * 
 * @param user_history 
 * @param user_embedding 
 * @param local_user_embedding 
 * @param local_item_embedding 
 * @param prediction 
 * @param local_gramian 
 * @param local_global_gramian 
 * @param reg 
 * @param unobserved_weight 
 * @return const Recommender::VectorXf 
 */
...
class IALSppRecommender : public Recommender {
 public:
  ...
  void Train(const Dataset& data) override {
    ...
    // Generating sparse prediction matrix
    ...
    for (const auto& user_and_history : data.by_user()) {
      ...
      for (const auto& item_and_rating_index : user_and_history.second) {
        // item_and_rating_index.second is the nnz_id
        // updating that value with item_embedding * user_embedding
        prediction.coeffRef(item_and_rating_index.second) =
            item_embedding_.row(item_and_rating_index.first).dot(user_emb);
      }
    }
    ...
    for (int i = 0; i < num_threads; ++i) {
      threads.emplace_back(std::thread([&]{
        while (true) {
          ...
          // get the item ids associated with that user
          ...
          // get the subvector of the user embedding
          VectorXf old_local_user_emb = old_user_emb.segment(
              block_start, block_end - block_start);

          // run projection and update the subvector
